...
struct pls {
  ...
  uint32_t magic;
#endif
  char str[]; // Dichiarazione in stile C99, non si sa quanti byte verranno
};
...
int main(void) {
  ...
  printf("%s\n", global_string);
  ps_release(mystr);
  // Se metto più release dopo che è già stata fatta la free usiamo il campo magic
  // Per capirlo e far si che non si ottengano errori runtime
  // ps_release(mystr);
  return 0;
}